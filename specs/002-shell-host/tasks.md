...
- [ ] T062 [US5] Restore the `apps` argument in the root `package.json` `check:boundaries` script: `depcruise apps packages --config .dependency-cruiser.js`
- [ ] T063 [US5] Run `pnpm check:boundaries` and confirm it passes against the real `apps/shell` source (`FR-020` first half)
- [ ] T064 [US5] **Prove the gate can fail**: add a relative import from `apps/shell/src` reaching into a path shaped like another app (e.g. `../../dashboard/src/exposed/App`), run `pnpm check:boundaries`, confirm it exits non-zero and names `no-cross-app-relative-imports`, then revert (`FR-020` second half, `SC-006` — the first time in the project's history this gate has been able to fail)
- [ ] T065 [US5] **Prove the internal-boundary rule too**: add an import from outside `apps/shell/src/internal/` reaching into `apps/shell/src/internal/federation/manifest.ts` directly (bypassing `exposed/`), confirm `pnpm check:boundaries` fails naming `no-reaching-into-internal`, then revert

**Checkpoint**: Both dependency-cruiser rules are proven able to fail, closing issue #2
